...
// ===========================================================================
// Kotlin via the semanticdb-kotlinc compiler plugin + scip-java index-semanticdb
// ===========================================================================
//
// scip-kotlin isn't on Maven Central (it ships only via the
// scip-kotlin git repo + sbt publishM2). The cross-lang test
// exercises the principled "build it from source" path: we
// detect a locally-published `semanticdb-kotlinc` JAR in
// ~/.m2/repository/com/sourcegraph/semanticdb-kotlinc/ and wire
// it into a Gradle Kotlin project as a Kotlin compiler plugin,
// then convert the emitted *.semanticdb files into a SCIP
// index via `scip-java index-semanticdb`. scry consumes that
// index.scip via auto-discovery in the usual way.
//
// To build semanticdb-kotlinc locally (one-time setup):
//   git clone https://github.com/sourcegraph/scip-kotlin
//   cd scip-kotlin && sbt publishM2
#[test]
fn kotlin_scip_end_to_end() {
    let scip_java = match find_tool(&["scip-java"], &["/mnt/agent/tools/bin"]) {
        Some(p) => p,
        None => {
    ...
    if find_tool(&["gradle"], &["/mnt/agent/tools/bin"]).is_none() {
        eprintln!("SKIP: gradle not on PATH");
        return;
    }
    // Find the most-recent semanticdb-kotlinc snapshot under ~/.m2.
    let m2_dir = match std::env::var("HOME").ok().map(|h| {
        PathBuf::from(h).join(".m2/repository/com/sourcegraph/semanticdb-kotlinc")
    }) {
        Some(p) if p.is_dir() => p,
        _ => {
            eprintln!("SKIP: semanticdb-kotlinc not published locally; \
                       run `sbt publishM2` in a scip-kotlin checkout first");
            return;
        }
    };
    let semdb_jar = std::fs::read_dir(&m2_dir).ok().into_iter().flatten()
        ...
        .flat_map(|d| std::fs::read_dir(d).ok().into_iter().flatten()
            ...
            .filter(|p| {
                ...
                n.starts_with("semanticdb-kotlinc") && n.ends_with(".jar")
                    && !n.contains("sources") && !n.contains("javadoc")
            })
            .collect::<Vec<_>>().into_iter()
        )
        .next();
    let semdb_jar = match semdb_jar {
        Some(p) => p,
        None => {
            eprintln!("SKIP: no semanticdb-kotlinc-*.jar found under {}",
                      m2_dir.display());
            return;
        }
    };
    ...
    let src = base.join("src/main/kotlin/demo");
    let idx = base.join("idx");
    std::fs::create_dir_all(&src).unwrap();
    std::fs::write(base.join("settings.gradle.kts"),
        "rootProject.name = \"fixture\"\n").unwrap();
    let semdb_path = semdb_jar.display().to_string();
...
val sdbTargetRoot = layout.buildDirectory.dir("semanticdb-targetroot")

tasks.withType<org.jetbrains.kotlin.gradle.tasks.KotlinCompile>().configureEach {{
    doFirst {{ sdbTargetRoot.get().asFile.mkdirs() }}
    compilerOptions {{
        freeCompilerArgs.addAll(provider {{
            val sourceroot = projectDir.absolutePath
            val target = sdbTargetRoot.get().asFile.absolutePath
            listOf(
                "-Xplugin={semdb_path}",
                "-P", "plugin:semanticdb-kotlinc:sourceroot=$sourceroot",
                "-P", "plugin:semanticdb-kotlinc:targetroot=$target",
            )
        }})
    }}
}}
"#);
    std::fs::write(base.join("build.gradle.kts"), build_gradle).unwrap();
    // Multi-line Kotlin (scry's tree-sitter spec captures defs only
    // when they span multiple lines — a single-line `class Foo { fun
    // bar() {} }` collapses to one capture in tree-sitter-kotlin-NG).
    std::fs::write(src.join("Animal.kt"), r#"package demo
...
fun pet(a: Animal): String {
    return a.speak()
}
"#).unwrap();

    let gradle = find_tool(&["gradle"], &["/mnt/agent/tools/bin"]).unwrap();
    let r = Command::new(&gradle).args(["compileKotlin", "--no-daemon"])
        .current_dir(&base).output().expect("spawn gradle compileKotlin");
    if !r.status.success() {
        eprintln!("SKIP: gradle compileKotlin failed (likely Kotlin/JVM \
                   incompat in test env): {}",
                  String::from_utf8_lossy(&r.stderr));
        std::fs::remove_dir_all(&base).ok();
        return;
    }
    let semdb_root = base.join("build/semanticdb-targetroot");
    if !semdb_root.is_dir() {
        eprintln!("SKIP: semanticdb-kotlinc didn't emit any *.semanticdb");
        std::fs::remove_dir_all(&base).ok();
        return;
    }
    let scip_path = base.join("index.scip");
    let r = Command::new(&scip_java)
        .args(["index-semanticdb", "--output"]).arg(&scip_path).arg(&semdb_root)
        .current_dir(&base).output().expect("spawn scip-java index-semanticdb");
    if !r.status.success() || !scip_path.exists() {
        eprintln!("SKIP: scip-java index-semanticdb failed: {}",
                  String::from_utf8_lossy(&r.stderr));
        std::fs::remove_dir_all(&base).ok();
        return;
    }
    ...
    assert!(status.starts_with("v1,"),
        "scip_index health should be 'v1, …'; got: {status}");
    assert!(extract_v1_count(&status) >= 5,
        "semanticdb-kotlinc should yield >= 5 SCIP symbols from the fixture; \
         got: {status}");
