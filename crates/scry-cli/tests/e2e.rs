...
/// v0.1.60 — Java static-receiver capture. `Foo.bar()` now emits a
/// TypeUse ref to `Foo` in addition to the existing Call ref to `bar`,
/// so `scry callers Foo` (or `scry ref Foo`) catches the static-method
/// usage site. Filter: only emits when the receiver identifier is
/// PascalCase (Java class-naming convention), to avoid false positives
/// from local-var receivers like `myService.foo()`.
#[test]
fn java_static_receiver_capture() {
    let nanos = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos();
    let base = std::env::temp_dir().join(format!("scry-static-recv-{nanos}"));
    let src = base.join("src");
    let idx = base.join("index");
    std::fs::create_dir_all(&src).unwrap();
    // Foo has a static method; Caller invokes it via `Foo.staticBar()`
    // AND has a local var `myFoo` calling `myFoo.instanceBar()` —
    // the latter MUST NOT emit a TypeUse ref to `myFoo`.
...
"#).unwrap();
    ...
    // (1) `scry ref Foo` should include the static-receiver call site
    // (Caller.java line where `Foo.staticBar()` appears).
    let out = Command::new(scry_bin())
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    let lines: Vec<&str> = stdout.lines().filter(|l| !l.trim().is_empty()).collect();
    let refs: Vec<serde_json::Value> = lines.iter()
        .map(|l| serde_json::from_str(l).unwrap()).collect();
    // We expect at least one TypeUse ref to Foo from the static-call site.
    let static_recv_hits: Vec<&serde_json::Value> = refs.iter()
        .filter(|r| r["path"].as_str().is_some_and(|p| p.ends_with("/Caller.java"))
                    && r["name"].as_str() == Some("Foo"))
        .collect();
    assert!(!static_recv_hits.is_empty(),
        "expected at least one Foo ref from Caller.java (static-receiver capture); \
         got refs={refs:?}");

    // (2) Negative: `scry ref myFoo` must NOT have any hits — the
    // lowercase-receiver filter should drop it. There's no def of
    // `myFoo` either, so 0 hits is the right answer.
    let out = Command::new(scry_bin())
        .args(["ref", "myFoo", "--index"]).arg(&idx)
        .args(["--json", "--limit", "50"])
        .output().expect("spawn ref myFoo");
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
    // ref output is JSONL; empty stdout = 0 refs.
    let n_hits = stdout.lines().filter(|l| !l.trim().is_empty()).count();
    assert_eq!(n_hits, 0,
        "expected 0 refs to lowercase `myFoo` (filter must drop local-var receivers); \
         got {n_hits}: {stdout:?}");
